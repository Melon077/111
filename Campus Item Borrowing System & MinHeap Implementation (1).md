README
Table of Contents
Task 1: Campus Item Borrowing Management System
Overview
File Structure
Quick Start
Code Explanation
Task 2: MinHeap Implementation and Heap Sort
Overview
Quick Start
Code Explanation
...
Place all Task 1 files (item.py, user.py, borrow_system.py, main.py) in the same directory.
Launch the system via command line:
shell
python main.py
...
Save the heap_study.py file to your working directory.
Run the script directly:
shell
python heap_study.py
The script will output the test data and sorted result:
plaintext
...
Replaces the root with the last element (after popping the last element from the heap).
Calls heapify_down(0) to rebalance the heap.
...
Inserts all elements of the input array into the min-heap.
Repeatedly extracts the minimum element from the heap and appends it to the sorted array.
